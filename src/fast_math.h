...
#define FAST_MATH_FASTLOG2_DOWN (0)
#define FAST_MATH_FASTLOG2_UP   (1)

#define FAST_MATH_ROUND_UP(x, alignment)  \
    (((x) + (alignment) - 1) & ~((alignment) - 1))

#define FAST_MATH_ALIGN_SIZE       sizeof(void *)

#define FAST_MATH_ALIGNMENT(x, align) \
    (((x) % (align)) ? ((x) + ((align) - (x) % (align))) : (x))

unsigned int fast_math_fastlog2(size_t x, int round);
int          fast_math_is_prime(size_t x);
size_t       fast_math_find_prime(size_t x);
