...
void *
array_create(pool_t *p, uint32_t n, size_t size)
{
    array_t *a = NULL;
    ...
    if (p) {
        a = pool_alloc(p, sizeof(array_t));
        a->elts = pool_alloc(p, n * size);
    } else {
        a = memory_alloc(sizeof(array_t));
...
int 
array_init(array_t *array, pool_t *pool, uint32_t n, size_t size)
{
    if (!array || (n == 0) || (size == 0)) {
        return FAST_ERROR;
    }
...
void
array_inherit(void *pool, array_t *a1, array_t *a2, func_cmp func, func_inherit func2)
{
    size_t i ,j;
    void *v1,*v2;
