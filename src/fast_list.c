...
int
list_init(list_t *list, pool_t *pool, uint32_t n, size_t size)
{
    if (list->part.elts) {
        return FAST_ERROR;
    }
...
list_reset(list_t *list)
{
    ...
    last = &list->part;
    while (last) {
        //memory_zero(last->elts, last->nelts * list->size);
        last->nelts = 0;
        last = last->next;
    }
    list->last = &list->part;
...
list_push(list_t *l)
{
    ...
    if (l->pool) {
        last = pool_alloc(l->pool, sizeof(list_part_t));
        last->elts = pool_alloc(l->pool, l->nalloc * l->size);
    } else {
        last = memory_alloc( sizeof(list_part_t));
