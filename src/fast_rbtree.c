...
void
rbtree_init(_xvolatile rbtree_t *tree,
    rbtree_node_t *s, rbtree_insert_pt i)
{
...
void
rbtree_sentinel_init(rbtree_node_t *s)
{
    if (s) {
        rbtree_black(s);
    }
...
void
rbtree_insert(_xvolatile rbtree_t *tree,
    rbtree_node_t *node)
{
...
void
rbtree_insert_value(rbtree_node_t *temp,
...
void
rbtree_insert_timer_value(rbtree_node_t *root,
    rbtree_node_t *node, rbtree_node_t *sentinel)
{
    ...
    for ( ;; ) {
        ...
        if (!(*p)) {
            ...
            event = (event_t *)(last - offsetof(event_t, timer));
            
            fast_log_debug(FAST_LOG_DEBUG_EVENT, 0, 
                "add timer: invalid event %p", event);
        }
 #endif
...
void
rbtree_delete(_xvolatile rbtree_t *tree,
    rbtree_node_t *node)
{
    uint32_t           red;
    ...
    while (temp != *root && rbtree_is_black(temp)) {
        ...
        if (!w->left || !w->right) {
            ...
            event = (event_t *)(w - offsetof(event_t, timer));
            
            fast_log_debug(FAST_LOG_DEBUG_EVENT, 0, 
                "delete timer: invalid event %p", event);
        }
 #endif
