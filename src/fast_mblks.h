#ifndef __FAST_MBLKS_H__ 
#define __FAST_MBLKS_H__ 1
#include <stdlib.h>
#include "fast_lock.h"
...
#define LOCK_INIT(x) do  { \
    fast_atomic_lock_init(x); \
}while(0)

#define TRY_LOCK(x) do  { \
    fast_lock_errno_t error; \
    fast_atomic_lock_try_on((x), &error); \
}while(0)

#define LOCK(x) do {\
    fast_lock_errno_t error; \
    fast_atomic_lock_on((x), &error); \
}while(0)


#define UNLOCK(x) do { \
    fast_lock_errno_t error; \
    fast_atomic_lock_off((x), &error); \
}while(0)
